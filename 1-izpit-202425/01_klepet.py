...
Struktura podatkov (da si lažje predstavljaš):
Vsak pogovor je slovar:
pogovor = {
    'naslov': 'Kam gremo jest?',
    'sporocila': [
        ('Tea', 'jaz bi šla v menzo :thumbsup:'),
        ('Aljaž', 'meni v menzi ne diši...'),
        ('Martin', 'meni je vseeno...')
    ]
}


pogovor['naslov'] nam da naslov.
pogovor['sporocila'] nam da seznam parov (oseba, besedilo).


1. Podnaloga: stisni(pogovor)
💡 Kaj naloga želi?
Vrniti trojico (nabor s 3 elementi):
$$\text{(naslov, število sodelujočih oseb, skupno število sporočil)}$$
🧠 Kako razmišljamo?

Naslov: vzamemo pogovor['naslov'].
Število sporočil: je dolžina seznama sporočil $\rightarrow$ len(pogovor['sporocila']).
Število sodelujočih: ena oseba lahko pošlje več sporočil, zato moramo prešteti unikatne osebe.

Uporabimo množico (set), ki avtomatsko zavrže podvojene osebe:
sodelujoci = {oseba for oseba, besedilo in pogovor['sporocila']}
Število oseb je len(sodelujoci).
...
def stisni(pogovor):
    ...
    # Množica pobere le unikatna imena oseb
    sodelujoci = {oseba for oseba, besedilo in sporocila}
    
    return (naslov, len(sodelujoci), len(sporocila))


2. Podnaloga: klepetulja(pogovori)
💡 Kaj naloga želi?
Prejmemo seznam vseh pogovorov. Poiskati moramo osebo, ki je v vseh pogovorih skupaj poslala največ sporočil.
🧠 Kako razmišljamo?

Ustvarimo slovar števcev: stevec = {}.
Gremo z zankama čez vse pogovore in vsa sporočila v njih:

Za vsako oseba povečamo njen števec v slovarju:
stevec[oseba] = stevec.get(oseba, 0) + 1


Poiščemo ključ (osebo) z največjo vrednostjo:

max(stevec, key=stevec.get) vrne osebo z največ sporočili.
...
def klepetulja(pogovori):
    ...
    # Pregledamo vsak pogovor in vsako sporočilo
    for pogovor in pogovori:
        for oseba, besedilo in pogovor['sporocila']:
            stevec[oseba] = stevec.get(oseba, 0) + 1
            
    # Poiščemo osebo z največjim številom sporočil
    return max(stevec, key=stevec.get)


3. Podnaloga: custvencki(pogovori)
💡 Kaj naloga želi?
Vrniti množico (set) vseh unikatnih čustvenčkov, ki se pojavijo v vseh sporočilih.

Čustvenček se začne in konča z dvopičjem :, vmes pa so samo male angleške črke (npr. :thumbsup:, :cry:).


🧠 Kako razmišljamo? (2 preprosta načina)
1. način: Z regularnimi izrazi (re) – Najkrajše in najbolj elegantno
Vzorec r':[a-z]+:' pomeni: dvopičje, ena ali več malih angleških črk, dvopičje.
import re
...
def custvencki(pogovori):
    ...
    for pogovor in pogovori:
        for oseba, besedilo in pogovor['sporocila']:
            ...
            vsi_custvencki.update(najdeni)
            
    return vsi_custvencki

2. način: Brez uvoza knjižnic (s split(':')) – Če se na izpitu ne spomniš re
Ko niz "... :thumbsup: ..." razrežemo s split(':'), bodo vsi potencialni čustvenčki na lihih indeksih ($1, 3, 5, \dots$). Preverimo le, ali so vsi znaki male angleške črke:
def custvencki(pogovori):
    vsi_custvencki = set()
    ...
    for pogovor in pogovori:
        for oseba, besedilo in pogovor['sporocila']:
            deli = besedilo.split(':')
            # Vsebina med dvopičji se nahaja na lihih indeksih
            for i in range(1, len(deli), 2):
                vsebina = deli[i]
                # Preverimo, da ni prazno in so samo male angleške črke
                if vsebina.isalpha() and vsebina.islower() and vsebina.isascii():
                    vsi_custvencki.add(f":{vsebina}:")
...
set(): avtomatsko poskrbi za unikatne elemente (tako za štetje oseb kot za vračanje množice čustvenčkov).
max(slovar, key=slovar.get): najhitrejši način za pridobitev ključa z največjo vrednostjo v slovarju.
re.findall(r':[a-z]+:', tekst): najde vse podnize oblike :nekaj:.
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0Mjk2MywidXNlciI6MTE0NTR9:1x4BQq:RsEGMskevh0NNTDcPoRTf0XMlB2jiqv2wilAFtrJ_w0"
        try:
            pogovor1 = {
                'naslov': 'Kam gremo jest?',
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0Mjk2MSwidXNlciI6MTE0NTR9:1x4BQq:TmYlpnna3_kGHgy8q9KjFhCH3Sjn1BlIDftTFb_ly8w"
        try:
            pogovor1 = {
                'naslov': 'Kam gremo jest?',
                'sporocila': [('Tea', 'jaz bi šla v menzo :thumbsup:'), ('Aljaž', 'meni v menzi ne diši, a bi šlo raje v Hombre?'), ('Martin', 'meni je vseeno, zmente se')]
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0Mjk2MiwidXNlciI6MTE0NTR9:1x4BQq:fluANFVz4QNEB6lnhnBwUVJxzSX7BLYE-aa0cZrLWmw"
        try:
            pogovor1 = {
                'naslov': 'Kam gremo jest?',
                'sporocila': [('Tea', 'jaz bi šla v menzo :thumbsup:'), ('Aljaž', 'meni v menzi ne diši, a bi šlo raje v Hombre?'), ('Martin', 'meni je vseeno, zmente se')]
