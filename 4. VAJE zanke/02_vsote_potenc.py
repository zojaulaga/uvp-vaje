...
def vsota_prvih_kvadratov(n):
    vsota = 0
    for i in range(1, n+1): ##zgornaj meja je izkljucena
        vsota += i**2
    return vsota
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NzgsInVzZXIiOjExNDU0fQ:1w28t9:nLJGqz5FTASZvOYFXQUB2R1y6C79CBGGh_cGv4YWWUE"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NzksInVzZXIiOjExNDU0fQ:1w28t9:CLNjhyx2u0DkGe-m2DoExYMSWujtxhbCv8qM570ry2c"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODAsInVzZXIiOjExNDU0fQ:1w28t9:HKthjn3YWMn23UFzbjbEg05vaXNLvihedu_2vHgU1wY"
        try:
            try:
