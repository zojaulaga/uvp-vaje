...
# =============================================================================
def najvecji_clen(n):
    cleni = [n]  # 1. Začnemo s seznamom, v katerem je že začetni člen n

    while n != 1:
        n = naslednji_clen(n)
        cleni.append(n)  # 2. Vsak nov člen vržemo v seznam

    return max(cleni)  # 3. Vgrajena funkcija max poišče največjega!
...
#        else:
#            return max(dolzina_zaporedja(m, n+1))
#        
#def najdaljse_zaporedje(m, n):
#    naj_dolzina = 0
#    for x in range(m, n + 1):  # n + 1 zato, da vključimo tudi število n!
#        trenutna = dolzina_zaporedja(x)
#        if trenutna > naj_dolzina:
#            naj_dolzina = trenutna
#    return naj_dolzina

def najdaljse_zaporedje(m, n):
    vse_dolzine = []  # 1. Prazen seznam za shranjevanje dolžin

    for i in range(m, n + 1):  # 2. Za vsako število med m in n:
        d = dolzina_zaporedja(i)  # Izračunamo njegovo dolžino
        vse_dolzine.append(d)  # To dolžino shranimo v seznam

    return max(vse_dolzine)  # 3. Na koncu vrnemo največjo med vsemi dolžinami!
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODUsInVzZXIiOjExNDU0fQ:1w28t9:S4qChpiZ146OOmYEsa2-7tSuhg6mPi8mS1B9BJH1Lrs"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODYsInVzZXIiOjExNDU0fQ:1w28t9:Lcncq3rOJENt_texDs2zgc9fz7rz0PI5o_nrksoASgw"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODcsInVzZXIiOjExNDU0fQ:1w28t9:Sg0TfH1HRYV6VMm7kVpQDeREf4fGqA914nNfhpfzppY"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODgsInVzZXIiOjExNDU0fQ:1w28t9:AIt3oORBYy86txAVhven8_uNXSqz6ITxy8VwbUjmke0"
        try:
