...
#    else:
#        return False
# dober poskus ampak ni prau , ker ze na poti ne smejo prerast tega stevila

def oklepaji(niz):
    odprti = 0  # Števec začnemo pri 0

    for crka in niz:
        if crka == "(":
            odprti += 1  # Odprli smo nov oklepaj
        elif crka == ")":
            odprti -= 1  # Zaprli smo en oklepaj

            # Če je zaprtih več kot odprtih (števec gre v minus), je narobe:
            if odprti < 0:
                return False

    # Na koncu morajo biti VSI oklepaji zaprti (števec mora biti točno 0):
    return odprti == 0
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODksInVzZXIiOjExNDU0fQ:1w28t9:SfHjjzosG9bWmqWocfWcWtOAGmYMkloNb9fOaMNaUfY"
        try:
