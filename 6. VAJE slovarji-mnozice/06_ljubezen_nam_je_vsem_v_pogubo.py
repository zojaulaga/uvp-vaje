...
def ljubljeni(zaljubljeni):
    ljubljen = set()
    for oseba in zaljubljeni:
        for ljubljeni in zaljubljeni[oseba]:
            ljubljen.add(ljubljeni)
    return ljubljen

#def ljubljeni(zaljubljeni):
#    vsi_ljubljeni = set()
#    
#    # 1. Gremo čez vsako osebo v slovarju
#    for oseba in zaljubljeni:
#        # 2. Vzamemo množico ljudi, ki jih ta oseba ljubi:
#        tisti_ki_jih_ljubi = zaljubljeni[oseba]
#        
#        # 3. Vsakega od njih vržemo v našo končno množico:
#        for ljubljena_oseba in tisti_ki_jih_ljubi:
#            vsi_ljubljeni.add(ljubljena_oseba)
#            
#    return vsi_ljubljeni
# =====================================================================@001386=
# 3. podnaloga
...
def pari(zaljubljeni):
    ...
    for oseba_A in zaljubljeni:
        for oseba_B in zaljubljeni[oseba_A]:  # oseba A ljubi osebo B
            
            # 1. Ali oseba B sploh obstaja v slovarju in ali B ljubi A nazaj?
            # 2. Ali je oseba A po abecedi pred osebo B? (s tem preprečimo podvajanje)
            if oseba_B in zaljubljeni and oseba_A in zaljubljeni[oseba_B] and oseba_A < oseba_B:
                srecni_pari.add((oseba_A, oseba_B))
...
# =============================================================================
def ustrezljivi(ciljna_oseba, zaljubljeni):
    ustrezni = set()
    kandidati = {ciljna_oseba}  # začnemo pri osebi, ki ji vsi želijo ustreči
    
    while len(kandidati) > 0:
        trenutna = kandidati.pop()  # vzamemo eno osebo
        
        # Poiščemo vse, ki ljubijo to trenutno osebo:
        for oseba, koga_ljubi in zaljubljeni.items():
            if trenutna in koga_ljubi and oseba not in ustrezni:
                ustrezni.add(oseba)
                kandidati.add(oseba)  # zdaj moramo poiskati še tiste, ki ljubijo to osebo!
                
    return ustrezni

def ustrezljivi(oseba, zaljubljeni):
    vsi_ustrezljivi = set()
    iscemo = {oseba}  # začnemo pri osebi, ki ji želimo pomagati (npr. 'Cene')
    
    # Dokler imamo še koga za preveriti:
    while len(iscemo) > 0:
        trenutna = iscemo.pop()  # vzamemo eno osebo (npr. 'Cene')
        
        # Pogledamo čisto vse ljudi v slovarju:
        for clovek, koga_ljubi in zaljubljeni.items():
            
            # Če ta človek ljubi našo trenutno osebo IN še ni v ustrežljivih:
            if trenutna in koga_ljubi and clovek not in vsi_ustrezljivi:
                vsi_ustrezljivi.add(clovek)  # dodamo ga med ustrežljive
                iscemo.add(clovek)           # zdaj moramo preveriti še, kdo ljubi NJEGA!
                
    return vsi_ustrezljivi
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzg0LCJ1c2VyIjoxMTQ1NH0:1x3zmU:P7KSbofVN_NBJeIyCGKhdkl716lQbNHjRmhhdHSNMFA"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzg1LCJ1c2VyIjoxMTQ1NH0:1x3zmU:yvwUMJWv_ghPCUopaaKqvksV8pMHDZSyV0m-F4RCmxo"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzg2LCJ1c2VyIjoxMTQ1NH0:1x3zmU:yC5Ansu7Pd5szpgPO4_HvDnXrScclcykzoqPYyMQIfw"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzg3LCJ1c2VyIjoxMTQ1NH0:1x3zmU:6i-CyC3bzP3Si9owQ_rCEgvzwPkdrROtA6HVG5xl6eM"
        try:
