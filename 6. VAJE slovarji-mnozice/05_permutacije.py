...
def cikel(permutacija, x):
    sez = [x]                     # 1. Začnemo z začetnim številom x
    trenutni = permutacija[x]     # 2. Naredimo prvi skok
    
    # 3. Skačemo DOKLER ne pridemo spet nazaj na x:
    while trenutni != x:
        sez.append(trenutni)      # dodamo trenutno število v cikel
        trenutni = permutacija[trenutni]  # skočimo na naslednje število
...
def cikli(slovar):
    vsi_cikli = []
    porabljena = set()  # beležnica za števila, ki so že v kakšnem ciklu
    
    # sorted(slovar) poskrbi, da začnemo pri najmanjših številih: 1, 2, 3...
    for x in sorted(slovar):
        if x not in porabljena:
            trenutni_cikel = cikel(slovar, x)  # uporabimo našo funkcijo cikel!
            vsi_cikli.append(trenutni_cikel)   # dodamo med vse cikle
            
            # Vsa števila iz tega cikla označimo kot porabljena:
            for stevilka in trenutni_cikel:
                porabljena.add(stevilka)
                
    return vsi_cikli

#def cikli(slovar):
#    vsi_krogi = []       # Naš list papirja za zapisovanje krogov
#    nalepke = set()      # Rdeče nalepke za kuverte, ki smo jih že odprli
#    
#    # Gremo po vrsti čez kuverte: 1, 2, 3...
#    for kuverta in sorted(slovar):
#        
#        # Če kuverta ŠE NIMA nalepke:
#        if kuverta not in nalepke:
#            
#            # Uporabimo funkcijo cikel, da odpre kuverte in najde cel krog:
#            krog = cikel(slovar, kuverta)   # Za kuverto 1 dobimo: [1, 3]
#            
#            # Ta krog zapišemo na naš list:
#            vsi_krogi.append(krog)          # Na list napišemo [1, 3]
#            
#            # Nalepimo nalepke na vse kuverte iz tega kroga:
#            for k in krog:
#                nalepke.add(k)              # Nalepimo nalepko na 1 in na 3!
#                
#    return vsi_krogi
# =====================================================================@001383=
# 5. podnaloga
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzc5LCJ1c2VyIjoxMTQ1NH0:1x3zmU:vyRgm5DCxlezOl720wMDHh8uLhbPelzGrpmQVK1ZJP8"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzgwLCJ1c2VyIjoxMTQ1NH0:1x3zmU:UttxVTxjeFe0Zu7gGIN8oRHkTKXGpJuW3xvsKvRDfrc"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzgxLCJ1c2VyIjoxMTQ1NH0:1x3zmU:C9wFQPVeZqGAMlueylwfYhQ59MI7_b96r0qKAi7PeZY"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzgyLCJ1c2VyIjoxMTQ1NH0:1x3zmU:2YlJdfnscKNLPFebICHm7QRPfwF4lnJelt3qyMtH8tY"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzgzLCJ1c2VyIjoxMTQ1NH0:1x3zmU:zLR-o4BHIHCSu8WGZusGdQHqFhYafuzYk_cNHM13Rsc"
        try:
