...
def prestej_crke(geslo, niz):
    ...
    rezultat = []
    for crka in geslo:
        rezultat.append(niz.count(crka))
...
def sestej_stevke(stevke):
    ...
    # Gremo samo do predzadnjega elementa!
    for i in range(len(stevke) - 1): # ker cene pol ko prides do i + 1 nemroes 
        d = stevke[i] + stevke[i + 1]
        
        if d >= 10:
            seznam.append(d // 10)  # Desetica (npr. 1 pri 12)
            seznam.append(d % 10)   # Enotica (npr. 2 pri 12)
        else:
            seznam.append(d)
...
def ujemanje(oseba1, oseba2, geslo='LOVES'):
    skupaj = oseba1 + oseba2
    stevke = prestej_crke(geslo, skupaj)
    
    # Seštevamo sosednje števke, dokler ne ostaneta samo še 2 števki
    while len(stevke) > 2:
        stevke = sestej_stevke(stevke)
        
    # Sestavimo končni dvomestni odstotek (npr. [6, 3] -> 63)
    return stevke[0] * 10 + stevke[1]
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQxMywidXNlciI6MTE0NTR9:1w4gPR:XPbkRjN8TLDTfoUmvG4immRqgJRGbxkIJ0I8Nrh0mlk"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQxNCwidXNlciI6MTE0NTR9:1w4gPR:V4IbGfP79_S3l1ONiBMLF2JslVhOIvbYnvntohefZGU"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQxNSwidXNlciI6MTE0NTR9:1w4gPR:G3bAiLS8XA2teITQXOWWR6zmLAVovoIsRNJ_nkQlAHE"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQxNiwidXNlciI6MTE0NTR9:1w4gPR:OlnExnjFA24zdFOchCAFy8XYhjhMHXvJEV2Zx3NCByU"
        try:
