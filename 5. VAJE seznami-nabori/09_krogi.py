...
def v_uniji(x0, y0, krogi):
    for x, y, r in krogi:
        # Preverimo, če je točka v tem krogu
        if (x0 - x)**2 + (y0 - y)**2 <= r**2:
            return True   # Našli smo vsaj enega!
            
    return False          # Ni v nobenem krogu
# =====================================================================@001312=
# 2. podnaloga
...
def v_preseku(x0, y0, krogi):
    for x, y, r in krogi:
        # Preverimo, če točka NI v tem krogu
        if (x0 - x)**2 + (y0 - y)**2 > r**2:
            return False  # Takoj ko izpade iz enega kroga, ni v preseku!
            
    return True           # Leži v vseh krogih
# =====================================================================@001313=
# 3. podnaloga
...
def pravokotnik(krogi):
    ...
    for x, y, r in krogi:
        x_levi.append(x - r)      # leva meja kroga
        x_desni.append(x + r)     # desna meja kroga
        y_spodnji.append(y - r)   # spodnja meja kroga
        y_zgornji.append(y + r)   # zgornja meja kroga
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzExLCJ1c2VyIjoxMTQ1NH0:1w4gPR:vBKVS7Xkl5RR5aG-MyEQhCQsmmhwyM7DfPcLJ5jk8_o"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzEyLCJ1c2VyIjoxMTQ1NH0:1w4gPR:yHOPSeOlLecf8TBAPElNJuNITgT7k30WBwTZR6BLRzY"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzEzLCJ1c2VyIjoxMTQ1NH0:1w4gPR:2wDD-fIUgQ0tWwpaETeznD9r2PAFoZqh0HqXVH-tyP8"
        try:
