...
def je_permutacija(sez):
    n = len(sez)
    return sorted(sez) == list(range(1, n + 1))

def je_permutacija(sez):
    n = len(sez)
    # Preverimo vsako število od 1 do n
    for x in range(1, n + 1):
        if x not in sez:
            return False  # Če eno število manjka, ni permutacija
...
#     False
# =============================================================================
def je_seznam_ciklov(cikli):
    videni = []
    
    for skupina in cikli:
        for x in skupina:
            # 1. Če je število 0 ali negativno -> narobe!
            if x <= 0:
                return False
            # 2. Če smo to število že kdaj prej videli -> narobe!
            if x in videni:
                return False
            videni.append(x)
...
def urejeni_cikli(cikli):
    ...
    for cikel in cikli:
        # Prazne cikle in cikle dolžine 1 izpustimo
        if len(cikel) <= 1:
            continue
        
        # Poiščemo indeks najmanjšega elementa v ciklu
        min_idx = cikel.index(min(cikel))
        
        # Cikel zavrtimo tako, da je najmanjši element na začetku
        zavrteni_cikel = cikel[min_idx:] + cikel[:min_idx]
        novi_cikli.append(zavrteni_cikel)
        
    # Uredimo cikle po njihovem prvem elementu
    novi_cikli.sort(key=lambda c: c[0])
    return novi_cikli
# =====================================================================@013407=
# 4. podnaloga
...
#     [5, 1, 7, 4, 2, 6, 3, 8, 9]
# =============================================================================
def iz_ciklov(cikli, dolzina=None):
    # Določimo največji element v ciklih
    max_el = 0
    for cikel in cikli:
        for x in cikel:
            if x > max_el:
    ...
    # Če dolžina ni podana ali je premajhna
    if dolzina is None or dolzina < max_el:
        dolzina = max_el
        
    # Začnemo z identiteto [1, 2, 3, ..., dolzina]
    perm = list(range(1, dolzina + 1))
    
    # Uveljavimo premike iz ciklov
    for cikel in cikli:
        k = len(cikel)
        for i in range(k):
            od_kje = cikel[i]
...
def v_cikle(perm):
    ...
    for i in range(1, len(perm) + 1):
        if i not in obiskani:
            trenutni = i
            cikel = []
            while trenutni not in obiskani:
                obiskani.add(trenutni)
                cikel.append(trenutni)
                trenutni = perm[trenutni - 1]
            cikli.append(cikel)
...
def inverz_perm(perm):
    # Pripravimo nov prazen seznam enake dolžine, poln ničel
    inverz = [0] * len(perm)
    
    # i je stara številka mesta (0, 1, 2...), kam_gre je vrednost
    for i in range(len(perm)):
        kam_gre = perm[i]
        # V novo hišo (kam_gre - 1) shranimo staro številko (i + 1)
        inverz[kam_gre - 1] = i + 1
...
def inverz_cikli(cikli):
    obrnjeni_cikli = []
    for cikel in cikli:
        # Obrnemo vrstni red elementov v ciklu
        obrnjeni_cikli.append(cikel[::-1])
        
    return urejeni_cikli(obrnjeni_cikli)
# =====================================================================@013411=
# 8. podnaloga
...
#     (4, 1, 1)
# =============================================================================
def ciklicni_tip(cikli, dolzina=None):
    max_el = 0
    for cikel in cikli:
        for x in cikel:
            if x > max_el:
    ...
    # Poiščemo dolžino najdaljšega cikla
    max_len = 1
    for cikel in cikli:
        if len(cikel) > max_len:
            max_len = len(cikel)
            
    # Seznam za štetje ciklov dolžin 1, 2, ..., max_len
    stevci = [0] * max_len
    ...
    # Vsi preostali elementi so cikli dolžine 1
    stevci[0] = dolzina - v_ciklih_vecih
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQwNCwidXNlciI6MTE0NTR9:1w4gPR:qFAVz03u8VBG67CeOd5fsdHcyXb2ylusUee4K7XYHjU"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQwNSwidXNlciI6MTE0NTR9:1w4gPR:ZyNMKOcuq8_l55P2rxJaDXYOSjAvMZYCZ1HBDPSMNPc"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQwNiwidXNlciI6MTE0NTR9:1w4gPR:KAxWMmWv6yz3DknJyp5HPDbaTlMHuH18yLS3DB22wDc"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQwNywidXNlciI6MTE0NTR9:1w4gPR:ocQChAV5u_IDnq8bUKfndrtCQy4JDVCIdmJFwppSyKs"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQwOCwidXNlciI6MTE0NTR9:1w4gPR:A6q9YHICTyyOzu5UTlDmh_vm_WAOPo9-UvJtxrLBBOY"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQwOSwidXNlciI6MTE0NTR9:1w4gPR:Lb4RQR-JIZy_fRCl1GD2IHPvx4M0eDRniI-eaQj96ho"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQxMCwidXNlciI6MTE0NTR9:1w4gPR:RwPTuY_yeD3Gk-Vk-arLvfCLePdKzaC987-TTDfkYvM"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQxMSwidXNlciI6MTE0NTR9:1w4gPR:2obvjjYqedqBj97Iyq6dNGr7gGHas1764HzxYb0GcJ4"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQxMiwidXNlciI6MTE0NTR9:1w4gPR:ReAiWFlNnsevzZkAxoXI5npm-adsZzkOco0K3Y3Cte0"
        try:
