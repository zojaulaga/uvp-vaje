...
def razpolovi_seznam(seznam):
    ...
    prvi = seznam[:sredina]
    drugi = seznam[sredina:]
    return (prvi, drugi)

#def razpolovi_seznam(sez):
#    sredina = len(sez) // 2
#    return (sez[:sredina], sez[sredina:])
# =====================================================================@009770=
# 2. podnaloga
...
def zamenjaj_elementa(sez, i, j):
    if i > len(sez) or j > len(sez):
        return sez
    
    nov = sez[:] #kopija s = sez.copy()
...
def zamenjaj_elementa(sez, i, j):
    ...
    # Preverimo, ali sta oba indeksa znotraj veljavnih meja seznama
    if 0 <= i < n and 0 <= j < n:
        nov = sez[:]  # Naredimo kopijo seznama
        nov[i], nov[j] = nov[j], nov[i]  # Zamenjamo elementa
        return nov
    else:
        return sez  # Če indeks ni veljaven, vrnemo kar originalen seznam
# =====================================================================@009771=
# 3. podnaloga
...
def porezani_podseznami(seznam):
    ...
    for i in range(len(seznam) + 1):
        rezultat.append(seznam[i:]) ##doda element??)
...
def najvecji_element(seznam):
    ...
    najvecji = seznam[0] ## najvecji = None   for a in sez: if a in sez 
...
def najvecji_element(sez):
    ...
    for element in sez:
        if element > najvecji:
            najvecji = element  # Shranimo nov največji element
            
    return najvecji  # Vrnemo šele, ko zanka pregleda vse elemente
# =====================================================================@020116=
# 5. podnaloga
...
##ta seznam nam na koncu vrne
## za vsak seznam v seznamu
## dodaš na koncu seznamu ta s ( novi seznam)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NzY5LCJ1c2VyIjoxMTQ1NH0:1w4gPR:hVuaAUlFJ4Yhx8VUSQwb7p3U_vXRiMYACFVOeeJo-9w"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NzcwLCJ1c2VyIjoxMTQ1NH0:1w4gPR:6uOG_bvdAvyxCDQ3adV4RDQKl1b4EVGqYjrGQC8TfvM"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5NzcxLCJ1c2VyIjoxMTQ1NH0:1w4gPR:EuQCDxuEAyoTlRE39JWW8kDC6ZrtnklcQl3ydqkmEZU"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODEyLCJ1c2VyIjoxMTQ1NH0:1w4gPR:wgLUVIvaDRXC3Jfao_JpV2nv9ALwfgz1mMjI_xgjzWs"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDExNiwidXNlciI6MTE0NTR9:1w4gPR:CLL5pGZZaymKMaoz07VpPRivk6gmnOZuU4XYnDdFpcA"
        try:
