...
def naivna_resitev(niz):
    ...
    for znak in niz:
     ...
     return stevilo == 0 and k == 0 and t == 0
 
 def naivna_resitev(niz):
    okrogle = niz.count("(") == niz.count(")")
    zavite = niz.count("{") == niz.count("}")
    oglate = niz.count("[") == niz.count("]")
    
    return okrogle and zavite and oglate
...
def gnezdeni_oklepaji(niz):
    sez = []  # Naša beležka za odprte oklepaje
    
    for znak in niz:
        # Če je katerikoli uklepaj, si ga samo zapomnimo na koncu seznama
        if znak == "(" or znak == "[" or znak == "{":
            sez.append(znak)
            
        # Če je zaklepaj ')':
        elif znak == ")":
            # Če je seznam prazen ALI pa zadnji NI '(' -> narobe!
            if len(sez) == 0 or sez[-1] != "(":
                return False
            sez.pop()  # Odstranimo zadnjega
    ...
    # Na koncu preverimo, ali je seznam prazen (vsi so se zaprli)
    return len(sez) == 0
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDI2OSwidXNlciI6MTE0NTR9:1w4gPR:pG4pmlCnVD1ByntLnO5SXuxye5QH6LxdBtTUnJdfsY0"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzM5NCwidXNlciI6MTE0NTR9:1w4gPR:nM5LgkQjeeNNhIze3K8YxZiLx8aVVMlApMR_u9O57ko"
        try:
