...
#     ['nepoliticno.txt']
# =============================================================================
Recept za 1. podnalogo: Branje $\rightarrow$ Sprememba $\rightarrow$ Pisanje
Kadar naloga zahteva, da prebereš eno datoteko, nekaj spremeniš in shraniš v drugo, je postopek vedno:

Odpri vhodno datoteko za branje (open(vhod, encoding='UTF-8')).
Preberi celotno vsebino v niz z .read().
Spremeni niz:

Velike črke: .upper()
Zamenjava znakov (piko v klicaj): .replace('.', '!')


Odpri izhodno datoteko za pisanje (open(izhod, 'w', encoding='UTF-8')).
Zapiši spremenjeno vsebino z .write(...).


Recept za 2. podnalogo: Filtriranje datotek v seznamu
Kadar dobiš seznam imen datotek in moraš vrniti samo tiste, ki ustrezajo pogoju:

Pripravi prazen seznam za rezultat: primerne = [].
Z zanko for se sprehodi čez vsako ime datoteke.
Za vsako datoteko:

Odpri jo in preberi njeno vsebino (dat.read()).
Preštej klicaje z metodo niza: vsebina.count('!').
Preveri pogoj: če je število klicajev $\ge 3$, dodaj ime datoteke v seznam primerne.append(ime).
...
def prepisi_in_zagrozi(vhodna_datoteka, izhodna_datoteka):
    # 1. Preberemo vsebino vhodne datoteke
    with open(vhodna_datoteka, 'r', encoding='UTF-8') as dat_in:
        vsebina = dat_in.read()
    
    # 2. Spremenimo vsebino (velike črke in pike v klicaje)
    spremenjena = vsebina.upper().replace('.', '!')
    
    # 3. Zapišemo spremenjeno vsebino v izhodno datoteko
    with open(izhodna_datoteka, 'w', encoding='UTF-8') as dat_out:
        dat_out.write(spremenjena)


# =====================================================================@020347=
# 2. podnaloga
def groznje_z_dovolj_klicaji(seznam_datotek):
    rezultat = []
    
    # Sprehodimo se po vsakem imenu datoteke v seznamu
    for ime_datoteke in seznam_datotek:
        with open(ime_datoteke, 'r', encoding='UTF-8') as dat:
            vsebina = dat.read()
            # Preštejemo klicaje in preverimo, ali jih je vsaj 3
            if vsebina.count('!') >= 3:
                rezultat.append(ime_datoteke)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDM0NiwidXNlciI6MTE0NTR9:1x41tG:s4kgCUc2S6ax5eKVNie6lIukbAhKIJQZjr7-Rfv8kqg"
        try:
            test_cases = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDM0NywidXNlciI6MTE0NTR9:1x41tG:NSkFlaOS3HcyNeWxPsuTZb-OrFIYPSodCRC99Q01hxI"
        try:
            test_cases = [
